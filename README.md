...
- 约 6.5 GB 可用磁盘（数据 200 MB + BGE-M3 模型 2.3 GB + Chromium 130 MB + Python 依赖 ~1.5 GB）
...
# 装 Playwright Chromium（用于 web fallback 绕过 mcmod 反爬，约 130 MB）
# 默认装到 D:/playwright-browsers，路径在 .env.example 的 PLAYWRIGHT_BROWSERS_PATH
uv run playwright install chromium
```

### 1.5（可选）配置代理池

**仅当你希望 `web_fallback` 在线查询 mcmod 时可用**：在 `.env` 取消注释并填入 KDL 或同类代理池的凭据：

```
PROXY_API_URL=https://dps.kdlapi.com/api/getdps/?secret_id=xxx&...&num=1&sep=1
PROXY_USER=your_username
PROXY_PASS=your_password
```

不配也能跑——但 web fallback 永远返回 "search failed"。知识库内的问题完全不受影响。

> 为什么要代理？mcmod.cn 对**自动化访问**会封禁 IP（即使是浏览器 UA + 真实 JS 执行）。每次走新代理 IP 是当前合规的工程方案。

...
- **自己使用**：你需要自行获取/构建模组数据并导入 SQLite (`data/minemate.db`)，
  表结构见 `pipeline/storage/models.py`；向量索引通过 `make build-index` 生成
- **数据来源**：mcmod.cn（遵守该站点 robots.txt 和服务条款）
...
**Q: 数据从哪来？**
A: 本项目不包含模组数据，也不提供爬虫脚本。数据来源为 mcmod.cn，使用者自行负责数据获取与合规。
